...
export runthis="g++ -std=c++20 *filename*.cpp camera.cpp ../Thirdparty/GLAD/src/*.c -I../Thirdparty/GLAD/include/ -I../Thirdparty/glm-master -o prog -lSDL2 -ldl"
$runthis
./prog
*/
...
void PreDraw(){
    ...
    // rotation
              model = glm::rotate(model, glm::radians(g_uRotate), glm::vec3(glm::cos(glm::radians(theta)), glm::sin(glm::radians(theta)), 0.0f));
    // scaling
    ...
    GLint u_Viewlocation = glGetUniformLocation(shader_pipeline_program, "u_ViewMatrix");

    if(u_Viewlocation >= 0){
        glUniformMatrix4fv(u_Viewlocation, 1, GL_FALSE, &view[0][0]); 
    } else {
        std::cout << "Could not find u_View, maybe a mispelling?\n" << std::endl;
        exit(EXIT_FAILURE);
    }
